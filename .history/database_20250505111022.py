...
class DBManager:
    ...
    def add_schedule(self, target_date):
        try:
            self.cursor.execute('''INSERT INTO schedules (target_date)
                                VALUES (?)''', (target_date,))
            self.conn.commit()
            return True
        except sqlite3.Error as e:
    ...
    def get_all_schedules(self):
        self.cursor.execute('''SELECT id, target_date FROM schedules ORDER BY target_date''')
